...
{	
	...
	HAL_UART_Receive_IT(&UART5_Handler, (u8 *)aRxBuffer, RXBUFFERSIZE);//该函数会开启接收中断：标志位UART_IT_RXNE，并且设置接收缓冲以及接收缓冲接收最大数据量
...
void HAL_UART_MspInit(UART_HandleTypeDef *huart)
{
	...
	if(huart->Instance==UART5)//如果是串口1，进行串口1 MSP初始化
	{
		...
		GPIO_Initure.Pin=GPIO_PIN_12;			//PC12-- UART5_Tx
		...
		GPIO_Initure.Pin=GPIO_PIN_2;			//PD2--UART5_Rx
...
{ 
	...
	timeout=0;
	while(HAL_UART_Receive_IT(&UART5_Handler, (u8 *)aRxBuffer, RXBUFFERSIZE) != HAL_OK)//一次处理完成之后，重新开启中断并设置RxXferCount为1
	{
	 timeout++; //超时处理
	 if(timeout>HAL_MAX_DELAY) break;	
	}
